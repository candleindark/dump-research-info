...
This script intentionally does not promote records into ``data/`` or call the
validator. Its transform output uses the exact class-file layout expected by
``dump-research-info`` and is suitable for review and validation.
"""
...
def fetch_paginated(
    api_root: str, endpoint: str, user_agent: str
    ...
    while True:
        ...
        urls.append(url)
        if version := headers.get("last-modified-version"):
            versions.append(version)

        total = int(headers.get("total-results", len(records)))
        if not payload or len(records) >= total:
            break
        start += len(payload)

    return records, versions, urls


def command_fetch(args: argparse.Namespace) -> None:
    api_root = f"https://api.zotero.org/groups/{args.group_id}"
    collections, collection_versions, collection_urls = fetch_paginated(
        api_root, "collections", args.user_agent
    )
    items, item_versions, item_urls = fetch_paginated(
        api_root, "items/top", args.user_agent
    )
    versions = [int(value) for value in collection_versions + item_versions if value]
    snapshot = {
        "source": {
            "api_root": api_root,
            ...
            "item_urls": item_urls,
            "library_version": max(versions) if versions else None,
            "zotero_api_version": ZOTERO_API_VERSION,
        },
        "collections": sorted(collections, key=record_key),
        "items": sorted(items, key=record_key),
    }
...
def source_items(snapshot: dict[str, Any]) -> list[SourceItem]:
    ...
    for item in snapshot.get("items", []):
        ...
        selected = bool(
            item_collections
            and any(name.casefold() != "external" for name in item_collections)
        )
        result.append(
            SourceItem(
                item=item,
...
def classify(item: SourceItem) -> tuple[str | None, str | None, str | None]:
    ...
    collection_set = {name.casefold() for name in item.collections}
...
def extract_isbns(value: Any) -> list[str]:
    ...
    normalized = {
        re.sub(r"[^\dXx]", "", candidate).upper() for candidate in candidates
    }
...
def command_transform(args: argparse.Namespace) -> None:
    ...
    for group_key in sorted(groups):
        ...
        preferred, _, kind, _ = max(
            eligible, key=lambda entry: completeness(entry[0])
        )
    ...
    report["counts"] = {
        "candidate_records": {
            name: len(records) for name, records in output.items()
        },
...
def build_parser() -> argparse.ArgumentParser:
    ...
    fetch.add_argument(
        "--user-agent",
        default="dump-research-info/0.1 (CON metadata ingestion)",
    )
